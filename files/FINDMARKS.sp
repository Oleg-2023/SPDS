...
begin
  /* Procedure Text */
  /* Если значение входных параметров начинается с '%' ищется вхождение параметра во всей строке */
  /* По умолчанию ищется совпадение по началу строки */
  /* '%'-добавляем в конец каждого  параметра */
  /**/
 ...
 /* Недолжно быть никогда */
  if (check_obj=0) then begin
    SUSPEND;
    EXIT;
  end
  /**/
  for select PO.genproj_id as GENPROJ_ID, /*Всегда выбираем*/
